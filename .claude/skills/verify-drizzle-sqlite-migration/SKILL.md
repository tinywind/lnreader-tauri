...
Use when Drizzle schema, generated SQLite migrations, SQL bridge code, backup DB gather/apply logic, or library/plugin persistence changes.

## Checks

- Review edited schema and migration files first.
- Treat the repo as SQLite + Drizzle, not Flyway/PostgreSQL.
- Use local naming consistency and existing domain names; do not import DoctorCheck `*_uid` rules.
...
- If plugin persistence changes, check Browse/search/install restart behavior against the current repository status docs.
