...
- Persistence should stay on SQLite through `src/db/client.ts`, `@tauri-apps/plugin-sql`, query helpers, and `src/db/schema.ts` / `drizzle/` migrations. Direct `getDb()` use is acceptable only at explicit persistence boundaries such as backup snapshot/apply, library update checks, plugin-to-library import orchestration, or a justified Rust-side storage boundary.
