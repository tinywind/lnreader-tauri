...
| `ImageBackground` + overlay + gradient | CSS: an `<img>` covered by `position: absolute; inset: 0; background: rgba(theme.bg, .55)` and a sibling `linear-gradient(transparent → bg)`. **v0.1 decision (locked, see `prd.md §9` Sprint 6)**: apply real `backdrop-filter: blur(20px) brightness(0.55) saturate(120%)` on the overlay layer. Upstream did not have this; the rewrite adopts it as a one-time visual upgrade. Pure CSS, ~3 lines, zero ongoing maintenance. Provide a graceful fallback for browsers without `backdrop-filter` support: keep the alpha overlay alone (acceptable parity with upstream). |
